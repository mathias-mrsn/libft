...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/29 09:36:31 by mamaurai          #+#    #+#             */
/*   Updated: 2021/11/29 10:56:14 by mamaurai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	__ft_get_arg_length__(t_options *options, t_argument *arg)
{
	if (ft_is_charset(options->conversion, "diu"))
		...
		arg->base_used = HEXA_LOWER;
	else if (options->conversion == 'X')
		arg->base_used = HEXA_LOWER;
	if (ft_is_charset(options->conversion, "diuxXp"))
	{
		if (options->conversion == 'p' && (long)arg->arg < 0)
			options->arg_length = (ft_nbrlen_b_u(arg->arg, arg->base_used));
		else if (arg->arg < 0)
			options->arg_length = (ft_nbrlen_b(-1 * arg->arg, arg->base_used));
		else
			options->arg_length = ft_nbrlen_b(arg->arg, arg->base_used);
	}
...
static void	__ft_get_arg__(t_options *options,
...
void	ft_parse_hub(t_options *options,
	const char **str, va_list settings, t_argument *arg)
{
